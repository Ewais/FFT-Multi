...
void *T1_FFT_Decomp_ThreadFn (void *input)
{
    ...
    */
    ...
    int ThreadSize = ((T1_FFT_Decomp_Input *)input)->ThreadSize;
    int ThreadID = ((T1_FFT_Decomp_Input *)input)->ThreadID;
    int Return;
#ifdef BAR
    pthread_barrier_wait(&ThreadStart);
#endif
    ...
    pthread_rwlock_unlock(&Lock);
        ...
        Return = pthread_create(&Threads[ThreadID+(ThreadSize/2)],NULL,T1_FFT_Decomp_ThreadFn,(void *) &In2);
        T1_FFT_Decomp_ThreadFn((void *)&In1);
        if (Return)
        {
